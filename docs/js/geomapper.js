...
// --- The Royal Lens: AR Scanner ---
let scannerActive = false;
let userHeading = 0; // 0 = North
let watchId = null;

function initScanner() {
    // 1. Add Button to control
    // Ideally we append to the existing control, but for simplicity we'll create a new one or hook into the existing one if we can see it. 
    // Since we can't easily query the existing Leaflet control from here without storing it, we'll create a second button in the same "topright" position which Leaflet will stack.

    const scannerControl = L.control({ position: 'topright' });
    scannerControl.onAdd = function (map) {
        const div = L.DomUtil.create('div', 'leaflet-bar');
        div.innerHTML = `
            <button class="reset-btn" id="btn-scanner" style="background:#2c3e50; color:white; border:1px solid #e74c3c;" onclick="toggleScanner()">
    ...
    if (window.map) scannerControl.addTo(window.map);
    ...
    const overlayHTML = `
        <div id="scanner-overlay" class="scanner-overlay">
            ...
            <div id="scanner-hud" class="scanner-hud"></div>
            <div class="scanner-compass" id="scanner-compass">
                <!-- Compass Ticks generated by JS -->
            </div>
            <button class="scanner-close" onclick="toggleScanner()">CLOSE SCANNER</button>
            
            <!-- Dev Simulator -->
            <div style="position:absolute; bottom:80px; left:50%; transform:translateX(-50%); background:rgba(0,0,0,0.5); padding:10px; border-radius:10px; color:white; text-align:center;">
                <label>Developer Simulator (Rotate Phone)</label><br>
                <input type="range" min="0" max="360" value="0" style="width:200px" oninput="simulateHeading(this.value)">
                <div id="debug-heading">0° N</div>
            </div>
        </div>
    `;
    ...
    window.toggleScanner = async () => {
        ...
        if (scannerActive) {
            ...
            try {
                ...
                console.warn("Camera denied or not available. Using Radar only.", e);
            }
            startSensors();
        } else {
    ...
    window.simulateHeading = (val) => {
        userHeading = parseInt(val);
        document.getElementById('debug-heading').textContent = `${userHeading}°`;
        updateAROverlay();
    };
}
...
function startSensors() {
    ...
    // Update loop
    if (!watchId) {
        watchId = setInterval(updateAROverlay, 100); // 10fps update
    }
}
...
function handleOrientation(event) {
    if (event.webkitCompassHeading) {
        ...
        // Android (roughly) - alpha is z-axis rotation, needs offset compensation usually
        // For 'Genius' demo simplicity we take absolute alpha or inverted
...
function updateAROverlay() {
    if (!scannerActive || !currentLatLng || !window.mapState.allMarkers) return;

    const hud = document.getElementById('scanner-hud');
    hud.innerHTML = ''; // Clear previous frame

    // Filter visible minerals (within 50km?)
    const markers = window.mapState.allMarkers;
    const nearby = markers.filter(m => {
        const dist = currentLatLng.distanceTo(m.getLatLng());
        return dist < 50000; // 50km range
    });

    nearby.forEach(m => {
        const bearing = getBearing(currentLatLng.lat, currentLatLng.lng, m.getLatLng().lat, m.getLatLng().lng);
        const diff = getAngleDiff(userHeading, bearing);

        // FOV is approx 60 degrees ( +/- 30 )
        if (Math.abs(diff) < 30) {
            // Render Card
            const dist = currentLatLng.distanceTo(m.getLatLng());
            const xPercent = 50 + (diff * 2); // Map -30..30 to roughly 0..100% screen width (simplified)

            // Y position based on distance (Closer = Lower/Bigger? Or just vertically centered?)
            // Let's keep it vertically centered for now, maybe varying slightly by distance
            const topPercent = 50;
            ...
            // Scale by distance
            const scale = Math.max(0.5, 1 - (dist / 50000));
            div.style.transform = `translate(-50%, -50%) scale(${scale})`;

            div.innerHTML = `
                <h3>${formatMineralType(m.mineralType)}</h3>
                <div>${(dist / 1000).toFixed(1)} km</div>
                <div style="font-size:10px; margin-top:5px;">${m.getTooltip()?.getContent() || 'Unknown'}</div>
            `;
            hud.appendChild(div);
        }
    });
    ...
    const compass = document.getElementById('scanner-compass');
    compass.innerHTML = '';
    // Draw North (0), East (90), etc based on userHeading
    // This is complex to render ticks dynamically, so for MVP we just show text
    const directions = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'];
    // ... (Skipping complex compass render for simplicity in this step)
    // Instead just center text
    compass.innerHTML = `<div style="text-align:center; color:white; line-height:50px; font-weight:bold;">HEADING: ${Math.round(userHeading)}°</div>`;
}
