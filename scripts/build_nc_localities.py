...
def fetch_census_places(state_fips="37", year=2025):
    ...
    if resp.status_code != 200:
        raise RuntimeError(f"Failed to download census TIGER {year} place shapefile from {url}")
...
def merge_and_export(osm_gdf, census_gdf, output_dir: Path):
    ...
    census_gdf = census_gdf[["GEOID", "NAME", "geometry"]].rename(columns={"NAME": "place_name", "GEOID": "geoid"})
    ...
    out_geo = joined[["osm_id", "osm_type", "final_name", "place", "population", "geoid", "geometry"]]
    out_geo["x_round"] = out_geo.geometry.x.round(6)
    out_geo["y_round"] = out_geo.geometry.y.round(6)
    out_geo["dup_key"] = out_geo["x_round"].astype(str) + "," + out_geo["y_round"].astype(str) + "," + out_geo["final_name"].fillna("")
    ...
    for _, row in out_geo.iterrows():
        ...
        popup = folium.Popup(html=f"<b>{row.final_name}</b><br>Place: {row.place}<br>Population: {row.population}<br>OSM ID: {row.osm_id}")
        folium.CircleMarker(coords, radius=3, color="blue", fill=True, popup=popup).add_to(m)
    m.save(html_path)
...
def main():
    ...
    parser.add_argument("--year", default=2025, type=int, help="Census TIGER year (e.g., 2025)")
    parser.add_argument("--non-interactive", action="store_true", help="Run fully non-interactively (default: false)")
    parser.add_argument("--pack-output", action="store_true", help="Zip output into a single archive after export")
    parser.add_argument("--use-sample", action="store_true", help="Use a small sample dataset instead of fetching from OSM/Census for quick testing")
    ...
    if args.use_sample:
        ...
        try:
            ...
            rows = [
                {"osm_id": 1, "osm_type": "node", "name": "Sample Place", "place": "city", "population": "1000", "tags": {}, "geometry": Point(-79.0, 35.5)}
            ]
            osm_gdf = gpd_mod.GeoDataFrame(rows, geometry="geometry", crs="EPSG:4326")
            census_gdf = gpd_mod.GeoDataFrame(columns=["GEOID", "NAME", "geometry"], geometry="geometry", crs="EPSG:4326")
        except Exception:
            ...
            gj = {
                ...
                "features": [
                    {
                        "type": "Feature",
                        "properties": {"osm_id": 1, "final_name": "Sample Place", "place": "city", "population": "1000"},
        ...
        print("Querying Overpass for place nodes/ways/relations in the state polygon...")
    ...
    if not args.use_sample:
        print("Downloading Census TIGER place shapefile (attempting the requested year, falling back if needed)...")
        census_gdf = None
        for y in range(args.year, 2019, -1):
            try:
                census_gdf = fetch_census_places(state_fips=args.state_fips, year=y)
                print(f"Census TIGER places loaded (year {y}): {len(census_gdf)} features")
                break
            except Exception as e:
    ...
    if census_gdf is None:
        print("No Census TIGER place shapefile could be fetched. Continuing with OSM points only.")
        if gpd is not None:
            ...
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
                for root, _, files in os.walk(outdir):
                    for f in files:
                        ...
                        zf.write(file_path, arcname=str(file_path.relative_to(outdir.parent)))
